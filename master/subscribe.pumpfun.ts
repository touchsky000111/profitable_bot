...
async function handleStream(client: Client, args: SubscribeRequest) {
    ...
    stream.on("data", async (data) => {
        ...
        const currentSlot = await web3Connection.getSlot("processed")
        const slotDiff = currentSlot - slot
        console.log("Diff => ", slotDiff)
...
async function subscribeCommand(client: Client, args: SubscribeRequest) {
    while (true) {
        try {
            console.log(chalk.green("💫") + ' Connecting to PumpFun Subscribe...');
...
export const startSubscribePumpfun = () => {
